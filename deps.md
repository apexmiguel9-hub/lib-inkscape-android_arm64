...
| sigc++ | 3.6.0 | <https://download.gnome.org/sources/libsigc++/3.6/libsigc++-3.6.0.tar.xz> (sha256 `c3d23b37dfd6e39f2e09f091b77b1541fbfa17c4f0b6bf5c89baef7229080e17`) | **dir oficial = `libsigc++` (NO `sigc++`)**: 3.8.0 no esta publicada ahi (`3.8/` = 404) y el archive de gitlab.gnome.org devuelve `302→Sign in` (HTML) en web/API/clone (run #19: `gzip: not in gzip format`). 3.6.0 es la ultima de download.gnome.org e Inkscape exige `sigc++-3.0>=3.6` → minimo exacto |
